...
<form method="POST" action="/quiz/create#Question-<?= count($Questions) - 2?>">
    ...
    <input type="text" name="header" id="header" value="<?= htmlspecialchars($Quez_name)?>"/>
    <label for="Description">Description:</label>
    <input type="text" name="description" id="description" value="<?= htmlspecialchars($Quez_description)?>"/>
    <hr>
    <?php foreach ($Questions as $id => $question) { ?>
        <h1>Question #<?= $id + 1?></h1>
        <label for="Question-<?= $id?>">Question</label>
        <input type="text" name="Question-<?= $id?>" id="Question-<?= $id?>" value="<?= htmlspecialchars($question["Question"])?>"/>
        <?php foreach ($question["Answers"] as $q_id => $q_answer) {?>
            ...
            <input type="text" id="Q-<?= $id?>-Answer-<?= $q_id?>" name="Q-<?= $id?>-Answer-<?= $q_id?>" value="<?= htmlspecialchars($q_answer)?>" placeholder="Type your question :))">
